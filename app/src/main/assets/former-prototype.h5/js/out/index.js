...
JS.status = $.extend({
    ...
    try {
        return JSON.parse(decodeURIComponent(location.href.match(/(?<=\?)(.*)(?=\#?)/)[0]));
    } catch (e) {}
}() || null);
...
$('.demo-layout').now(layout => ($(layout).find('.demo-header .mdl-layout-title').text(JS.translate(JS.status.mode === 'home' ? 'home' : JS.status.view)), $(layout).find('.demo-drawer .demo-navigation').prepend(function () {
    let icon_names = ['home', 'assignment', 'description'];
    let labels = ['home', 'papers', 'records'];
    let $as = Array.from({ length: 3 }).map((_, i) => $.tag('a', { class: 'mdl-navigation__link' }).append([$.tag('i', { class: 'mdl-color-text--blue-grey-400 material-icons', role: 'presentation' }).text(icon_names[i]), JS.translate(labels[i])]));
    JS.status.mode === 'home' ? $as[0].css({ background: 'rgb(0, 188, 212)' }) : $as[0].attr({ href: href() });
    $as.slice(1).forEach(($a, i) => function () {
        let label = labels[1 + i];
        return JS.status.view === label ? $a.css({ background: 'rgb(0, 188, 212)' }) : $a.attr({
    ...
    home: () => void 0,
    query: () => (({
        papers: () => $(content).append([$(JS.render.list([JS.translate('title'), JS.translate('description'), JS.translate('create time'), JS.translate('update time')], JS.data_samples.papers.list.map(line => [line.id, line.title, line.description, new Date(line.create_time).toLocaleString(), new Date(line.update_time).toLocaleString()]), 'paper')).now(table => Array.from($(table).find('tbody tr')).forEach(tr => function () {
        ...
        records: () => $(content).append([$(JS.render.list([JS.translate('paper'), JS.translate('title'), JS.translate('description'), JS.translate('create time'), JS.translate('update time')], data.list.map(line => [line.id, line.paper_id, line.title, line.description, new Date(line.create_time).toLocaleString(), new Date(line.update_time).toLocaleString()]), 'record')).now(table => Array.from($(table).find('tbody tr')).forEach(tr => function () {
            let tds = Array.from($(tr).find('td'));
    ...
    })[JS.status.view](), $(layout).find('div#view-source').append([$.tag('a', {
        class: 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored mdl-color-text--white'
    }).append($.tag('i', { class: 'material-icons' }).text('edit')), $.tag('a', {
        class: 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored mdl-color-text--white',
        href: href({
            mode: 'edit',
            view: {
    ...
    browse: () => (({
        paper: () => $(content).append([JS.render.title(JS.translate('basic information')), JS.render.keyValues([[JS.translate('title'), JS.data_samples.paper.title], [JS.translate('description'), JS.data_samples.paper.description], [JS.translate('create time'), new Date(JS.data_samples.paper.create_time).toLocaleString()], [JS.translate('update time'), new Date(JS.data_samples.paper.update_time).toLocaleString()]]), JS.render.title(JS.translate('content')), JS.form.renderBrowse(JS.data_samples.paper.data)])
    })[JS.status.view](), $(layout).find('div#view-source').append([$.tag('a', {
        class: 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored mdl-color-text--white',
        href: href({
            mode: 'edit',
    ...
    edit: () => (({
        paper: () => function () {
            ...
            return $(content).append([JS.render.title(JS.translate('basic information')), $(JS.render.keyValues([[JS.translate('title'), $.tag('input', { type: 'text', class: 'form-control input input-medium', input: 'title' }).now(input => data && $(input).val(data.title))], [JS.translate('description'), $.tag('textarea', { class: 'form-control input input-long', input: 'description' }).now(input => data && $(input).val(data.description))], [JS.translate('create time'), new Date(JS.data_samples.paper.create_time).toLocaleString()], [JS.translate('update time'), new Date(JS.data_samples.paper.update_time).toLocaleString()]])).now(table => $(table).addClass('basic-input-table')), JS.render.title(JS.translate('content')), $(JS.form.renderEdit(JS.empty(JS.status.id) ? null : JS.data_samples.paper.data)).now(table => $(table).addClass('content-input-table'))]);
        }()
    })[JS.status.view](), $(layout).find('div#view-source').append([$.tag('a', {
        class: 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored mdl-color-text--white',
    ...
    }).append($.tag('i', { class: 'material-icons' }).text('cancel')), $.tag('a', {
        class: 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored mdl-color-text--white'
    }).append($.tag('i', { class: 'material-icons' }).text('save')).click(e => ({
        paper: () => function () {
            let inputs = { id: JS.status.id };
            return $(content).find('.basic-input-table').now(table => function () {
                let title = $(table).find('[input=title]').val().trim();
                return title.length ? (inputs.title = title, inputs.description = $(table).find('[input=description]').val().trim(), inputs.data = {}, JS.form.getInputs(inputs.data, $(content).find('table.content-input-table').get(0)) && alert(JSON.stringify(inputs))) : alert(JS.translate('title') + '不能为空');
            }());
        }()
    })[JS.status.view]())]))
})[JS.status.mode]())));
